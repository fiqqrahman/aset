...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-4">
        <div>
            ...
            <p class="text-xs text-slate-500">Monitoring lalu lintas fetching data Kemendikdasmen ke Storage JSON Lokal.</p>
        </div>
        <div class="flex items-center gap-2">
            <!-- Form Action Sync -->
            <form method="POST" action="{{ route('admin.master-unit.sync') }}">
                @csrf
                <button type="submit" onclick="return confirm('Eksekusi Pipeline Ingestion Data dari Kemendikdasmen?')"
    ...
    <div class="grid grid-cols-12 gap-4 mb-4">
        ...
        <div
            ...
            <div>
                <div class="flex items-center justify-between">
                    <span class="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Total Staged
                        Record</span>
            ...
            <div class="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
                ...
                <span class="font-mono text-slate-700">storage/json/sekolah_smp.json</span>
            </div>
        </div>
        ...
        <div
            class="col-span-12 md:col-span-4 bg-white p-4 rounded-xl border border-slate-200 flex flex-col justify-between shadow-sm">
            <div>
                <span class="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Waktu Eksekusi
                    Terakhir</span>
                <p class="text-xl font-bold text-slate-900 mt-1">{{ $lastSync }}</p>
            </div>
            <div class="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
                <span>Mode Pipeline</span>
                <span class="font-medium text-slate-700">Manual / On-Demand</span>
            </div>
        </div>
    </div>

    <!-- Pipeline Configuration & Execution Log Section -->
    <div class="grid grid-cols-12 gap-4">
        <!-- Configuration Card (4 Cols) -->
        <div
            ...
            <div>
                ...
                <p class="text-[11px] text-slate-500 mb-4">Atur kriteria target data yang akan ditarik oleh engine.</p>
                ...
                <div class="space-y-3 text-xs">
                    ...
                    <div>
                        <label class="font-semibold text-slate-700 block mb-1">Jenjang Pendidikan</label>
                        <select
                            class="w-full px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-slate-800 font-medium focus:outline-none">
                            <option value="6" selected>SMP (Bentuk Pendidikan ID: 6)</option>
                            <option value="5" disabled>SD (Bentuk Pendidikan ID: 5) - Locked</option>
                            <option value="1" disabled>TK (Bentuk Pendidikan ID: 1) - Locked</option>
                        </select>
                    </div>
                    <div>
                        <label class="font-semibold text-slate-700 block mb-1">Target Storage File</label>
                        <input type="text" value="sekolah_smp.json" disabled
                            class="w-full px-3 py-1.5 bg-slate-100 border border-slate-200 rounded-lg font-mono text-slate-600">
                    </div>
                </div>
            </div>

            <div class="pt-4 border-t border-slate-100 mt-6">
                <span class="text-[10px] text-slate-400">Header HTTP & API Bearer tersimpan aman di Environment
                    Config.</span>
            </div>
        </div>

        <!-- Pipeline Execution Logs Table (8 Cols) -->
        <div
            ...
            <div>
                <div class="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                    <div>
                        <h3 class="text-xs font-bold text-slate-900 uppercase tracking-wider">Log Riwayat Pipeline & Sync
                            Job</h3>
                        <p class="text-[11px] text-slate-500">Audit trail eksekusi fetching data dari server Kemendikdasmen
                        </p>
                    </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr
                                ...
                                <th class="py-3 px-4">Target Param</th>
                                <th class="py-3 px-4">Record</th>
            ...
            <div class="p-3 border-t border-slate-100 bg-slate-50/30 text-right">
                <span class="text-[11px] text-slate-400">Menampilkan 2 Log Eksekusi Terbaru</span>
            </div>
        </div>
    </div>
@endsection
